...
spot_types = ["traversable", "obstacle"]
...
def display_grid():
    """Print the pathfinding grid on the console using conversion table characters"""
    for i in range(-1, rows):
        if i == -1:
            ...
            print('\n{}'.format(i), end='\t')
            for j in range(cols):
                print(type_conversion[grid[i][j].__repr__()], end='\t')
    ...
    while True:
        if len(open_set) > 0:
            ...
            for i in range(len(open_set)):
                if open_set[i].f < open_set[winner_index]:
                    winner_index = i

                # If we have a tie according to the standard heuristic
                # select the one with the longest known path (closer to goal)
                elif open_set[i] == open_set[winner_index]:
                    if open_set[i].g < open_set[winner_index].g:
                        winner_index = i

            # update current spot
            current_spot = open_set[winner_index]

            if open_set[winner_index] == end_spot:
                ...
                while temp.previous is not None:
                    ...
                    # #display path on the console
                    # for count, spot in enumerate(path):
                    #     print('{} {}'.format(count,spot.__str__()))

                print("Path found!")
                return True, path

            else:
                # move the best option to open set to closed set
                ...
                for current_neighbor in neighbors:
                    ...
                    if current_neighbor not in closed_set:
                        ...
                        elif temp_g >= current_neighbor.g:
                            # current neighbor is not a in a better path
                            continue
...
class Spot(object):
    ...
    def __init__(self, x, y, spot_type="traversable"):
        self.f = 0  # cost function
        self.g = 0  # actual cost from the starting node
...
if __name__ == '__main__':
    ...
    # for count, spot in enumerate(grid):
    #     print('{} {}'.format(count, spot.__str__()))

    # display_grid()
    print("Pathfinding...")
    feasible, solution = a_star()

    if feasible:
        for step, spot_index in enumerate(list(reversed(solution))):
            print('{} {}'.format(step, solution[spot_index].__str__()))
